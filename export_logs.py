...
def export_to_jsonl():
    ...
    with open(OUTPUT_PATH, 'w') as f:
        for row in rows:
            ...
            event_dict = {
                "event_id": f"evt-{row_dict.get('id', '000')}",
                "store_id": "ST1008",
                "camera_id": row_dict.get('camera_id') or "CAM_01",
                "visitor_id": str(row_dict.get('track_id') or "V000"),
                "event_type": row_dict.get('event_type') or "ENTRY",
                "timestamp": row_dict.get('timestamp') or "2026-06-04T00:00:00+00:00",
                "confidence": float(row_dict.get('confidence') or 0.95),
                "is_staff": bool(row_dict.get('is_staff') or False),
                "dwell_ms": int(row_dict.get('dwell_time') or 0) * 1000,
